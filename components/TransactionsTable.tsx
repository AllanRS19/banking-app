        ...
        <Table>
            ...
            <TableBody>
                {transactions.map((transact: Transaction) => {
                    const status = getTransactionStatus(new Date(transact.date));
                    const amount = formatAmount(transact.amount);

                    const isDebit = transact.type === 'debit';
                    const isCredit = transact.type === 'credit';

                    return (
                        <TableRow key={transact.id} className={`${isDebit || amount[0] === '-' ? 'bg-[#FFFBFA]' : 'bg-[#F6FEF9]'
                            } !over:bg-none !border-b-DEFAULT`}>
                            <TableCell className="max-w-[250px] pl-2 pr-10">
                                <div className="flex items-center gap-3">
                                    <h1 className="text-14 truncate font-semibold text-[#344054]">
                                        {removeSpecialCharacters(transact.name)}
                                    </h1>
                                </div>
                            </TableCell>
                            ...
                            <TableCell className="min-w-32 pl-2 pr-10">
                                {formatDateTime(new Date(transact.date)).dateTime}
                            </TableCell>

                            <TableCell className="pl-2 pr-10 capitalize min-w-24">
                                {transact.paymentChannel}
                            </TableCell>
